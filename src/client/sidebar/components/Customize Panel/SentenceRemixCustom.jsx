...
import { copyFirstSentenceAtom, pasteFirstSentenceAtom, contextRemixAtom } from '../state';

export default function SentenceRemixCustom() {
  const [copyFirstSentence, setCopyFirstSentence] = useAtom(
  ...
  const [pasteFirstSentence, setPasteFirstSentence] = useAtom(
  ...
  const [contextRemix, setContextRemix] = useAtom(contextRemixAtom);
  const handleContextSwitchChange = (event) => {
    setContextRemix(event.target.checked);
  }
    ...
    <div>
      <FormGroup>
        ...
        <FormControlLabel
          control={
            <Switch
              checked={copyFirstSentence}
              onChange={handleCopySwitchChange}
            />
          }
        ...
        <Tooltip title="Remixes will be contextually fine-tuned to text you've selected">
          <FormControlLabel
            control={
              <Switch 
                checked={contextRemix}
                onChange={handleContextSwitchChange}
              />}
            label="Suggest remixes based on what I'm currently typing"
          />
        </Tooltip>
      </FormGroup>
    </div>
